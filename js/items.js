		...
		super({
			...
			tableViewOptions: {
				...
				colTransforms: {
					...
					_crafted: {name: "Crafted Skill", transform: it => it.crafted?.skill},
					_dc: {name: "DC", transform: it => it.crafted?.dc},
		...
		if (item._fIsMundane) {
				...
				children: [
					e_({
						...
						children: [
							...
							e_({tag: "span", clazz: `col-4-4`, text: item.crafted?.skill.join(", ") || "\u2014"}),
							e_({tag: "span", clazz: `col-1-5 ve-text-center`, text: item.crafted?.dc || "\u2014"}),
			...
			const listItem = new ListItem(
				...
				item.name,
				{
					...
					source,
					type,
					cskill: item.crafted ? item.crafted.skill : "",
					dc: item.crafted ? item.crafted.dc : 0,
					value: item.value || 0,
					cost: item.value || 0,
					weight: Parser.weightValueToNumber(item.weight),
				},
			...
			const eleLi = e_({
				...
				children: [
					e_({
						...
						children: [
							e_({tag: "span", clazz: `col-3-5 pl-0 bold`, text: item.name}),
							e_({tag: "span", clazz: `col-3-9`, text: item.crafted?.skill.join(", ") || "\u2014"}),
							e_({tag: "span", clazz: `col-1-4 ve-text-center`, text: item.crafted?.dc || "\u2014"}),
							e_({tag: "span", clazz: `col-1-4 ve-text-center`, text:`${item.value || item.valueMult ? Parser.itemValueToFullMultiCurrency(item, {isShortForm: true}).replace(/\(.*\)/, "").replace(/ +/g, "\u00A0") : "\u2014"}`}),
